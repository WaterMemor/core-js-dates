...
function dateToTimestamp(date) {
  ...
  const differenceInMs = givenDate - epoch;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceInDays = Math.ceil(differenceInMs / 86400000) + 1;
...
function getCountWeekendsInMonth(month, year) {
  ...
  const lastDay = new Date(year, monthNew + 1, 0);
  const totalDays = lastDay.getDate();
  let weekendCount = 0;
  for (let day = 1; day <= totalDays; day += 1) {
    const currentDay = new Date(year, monthNew, day);
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDate <= endDate) {
        workSchedule.push(formatDateNew(currentDate));
      }
